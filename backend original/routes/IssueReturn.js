...
router.get('/view/:id', (req, res) => {
    return new Promise((resolve, reject) => {
        IssueReturn.findAll({ where: { deleteStatus: false, id: req.params.id } }).then(function (result) {
            sendSuccess(res, result);
        }).catch(function (err) {
            sendError(res, err);
        });
...
router.get('/particular/:id', (req, res) => {
    return new Promise((resolve, reject) => {
        IssueReturn.findAll({ where: { deleteStatus: false, id: req.params.id } }).then(function (result) {
            sendSuccess(res, result);
        }).catch(function (err) {
...
//table show only one record

router.get('/viewData3', (req, res) => {
    return new Promise((resolve, reject) => {
        IssueReturn.findAll({
            where: { deleteStatus: false }, order: [
                ['id', 'DESC']
...
router.delete('/delete/:id', (req, res) => { 
    ...
    return new Promise((resolve, reject) => {
        IssueReturn.destroy( { where: { id: req.params.id  } }).then(function (result) {
            sendSuccess(res, result);
        }).catch(function (err) {
            sendError(res, err);
        });
