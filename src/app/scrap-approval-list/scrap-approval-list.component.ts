...
export class ScrapApprovalListComponent implements OnInit {
  ...
  onKeyIns(x:any) { 
    ...
    this.dataservice.BreakageListDetails_postUser(this.registerDetails.InstrumentCode,this.collection).subscribe((data)=>{
  ...
  getUser(id: any) {
    // debugger;
    alert(id)
    this.router.navigate([`header/BreakageDetails2`], { queryParams: { id: id} });
  }
  
  update() {
    if ((this.registerDetails.Date === undefined) || (this.registerDetails.DocumentNo ==='') || (this.registerDetails.InstrumentCode ==='')|| (this.registerDetails.InstrumentName ==='')) {
      alert("Enter the Details");
    }
    else{
    // this.submitted = true;
    this.dataservice.ScrapApprovalList_updateSingleUser(this.registerDetails.data, this.registerDetails).subscribe(
      (data) => {
        // alert("Update");
        // console.log(data);
        if (data.data) {
          this.toastr.success('Updated!!!', 'ScrapApprovalList Updated Successfully.', {
            timeOut: 3000,
          });
          ...
          if (data.error.errors[0].validatorKey) {
            this.toastr.error('Error!!!', 'ScrapApprovalList Already Exists.', {
              timeOut: 3000,
            });
          }
        }
  ...
  public deleteUsers(id: string): void {
    console.log(id);
    this.dataservice.ScrapApprovalList_DeleteSingleUser(id,this.collection).subscribe((data: any) => {
      this.tabledata();
 ...
 if ((this.registerDetails.Date === undefined) || (this.registerDetails.DocumentNo ==='') || (this.registerDetails.InstrumentCode ==='')|| (this.registerDetails.InstrumentName ===''))
 {
    ...
    this.dataservice.ScrapApprovalList_postUser(this.registerDetails).subscribe(
      (data) => {
        // alert("Added");
        // console.log("Inserted"+data);
        if (data.data) {
          this.toastr.success('Created!!!', 'ScrapApprovalList  Created Successfully.', {
            timeOut: 3000,
          });
          let currentUrl = this.router.url;
          this.router.routeReuseStrategy.shouldReuseRoute = () => false;
